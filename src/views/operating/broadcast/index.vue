		...
		<div class="layout-padding-auto layout-padding-view">
			<el-row class="ml10" v-show="showSearch">
				<el-form :inline="true" :model="state.queryForm" ref="queryRef">
					<el-form-item :label="$t('journal.shop')" prop="jobName">
						<el-input :placeholder="$t('common.please') + $t('journal.shop')" @keyup.enter="getDataList" clearable
							v-model="state.queryForm.jobName" />
					</el-form-item>
					<el-form-item :label="t('param.status')" prop="jobStatus">
						<el-select :placeholder="t('common.select') + t('param.status')" v-model="state.queryForm.jobStatus">
							<el-option :key="index" :label="item.label" :value="item.value"
								v-for="(item, index) in job_status"></el-option>
						</el-select>
					</el-form-item>
					<el-form-item :label="t('broadcast.type')" prop="jobExecuteStatus">
						<el-select :placeholder="t('common.select') + t('broadcast.type')" v-model="state.queryForm.jobExecuteStatus">
							<el-option :key="index" :label="item.label" :value="item.value"
			...
			<el-table stripe :data="state.dataList" @selection-change="handleSelectionChange" style="width: 100%"
				...
				<el-table-column :label="t('refund.person')" fixed="left" prop="jobName" show-overflow-tooltip />
				<el-table-column :label="t('sysuser.phone')" prop="jobGroup" show-overflow-tooltip />
				<el-table-column :label="t('journal.shop')" prop="jobStatus" show-overflow-tooltip>
					<template #default="scope">
						<dict-tag :options="job_status" :value="scope.row.jobStatus"></dict-tag>
					</template>
				</el-table-column>
				<el-table-column :label="t('broadcast.type')" prop="jobExecuteStatus" show-overflow-tooltip>
					<template #default="scope">
						<dict-tag :options="job_execute_status" :value="scope.row.jobExecuteStatus"></dict-tag>
					</template>
				</el-table-column>
				<el-table-column :label="t('sysuser.lockFlag')" prop="startTime" show-overflow-tooltip />
				<el-table-column :label="$t('common.action')" fixed="right" width="120">
					<template #default="scope">
						<el-button @click="handleJobLog(scope.row)" text type="primary">{{ t('common.download') }}</el-button>
